    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            author: {
                ...
                url: String("https://scratchturbo.replit.app/profile?user=" + String(packet.user).substring(0, 50))
            },
...
const handleLogin = (req, res, local) => {
    ...
    UserManager.verifyCode(privateCode).then(response => {
        ...
        const invalidRedirect = local ? (
            ...
            response.redirect !== 'https://mainapi-scratchturbo.replit.app/api/users/login'
        );
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            fields: [
                ...
                {
                    name: "URL",
                    value: `https://scratchturbo.replit.app/profile?user=${packet.target}`
                },
            ],
            author: {
                ...
                url: String("https://scratchturbo.replit.app/profile?user=" + String(packet.target).substring(0, 50))
            },
            timestamp: new Date().toISOString()
        }, {
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            author: {
                ...
                url: String("https://scratchturbo.replit.app/profile?user=" + String(bannedUser).substring(0, 50))
            },
            timestamp: new Date().toISOString()
        }]
    });
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            fields: [
                ...
                {
                    name: "URL",
                    value: `https://scratchturbo.replit.app/profile?user=${reportedUser}`
                }
            ],
            author: {
                ...
                url: String("https://scratchturbo.replit.app/profile?user=" + String(reportedUser).substring(0, 50))
            },
            timestamp: new Date().toISOString()
        }]
    });
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            fields: [
                ...
                {
                    name: "URL",
                    value: `https://projects.scratchturbo.replit.app/${reportedProject}`
                }
            ],
            author: {
                ...
                url: String("https://scratchturbo.replit.app/profile?user=" + String(packet.username).substring(0, 50))
            },
            timestamp: new Date().toISOString()
        }]
    });
...
app.post('/api/projects/reject', async function (req, res) {
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            fields: [
                ...
                {
                    name: "Reject Type",
                    value: rejectHard ? 'hard-reject' : `soft-reject\nhttps://projects.scratchturbo.replit.app/${project.id}`
                },
                {
                    name: "Reason",
                    value: `${packet.reason}`
    ...
    if (rejectHard) {
        ...
        const projectFilePath = `./projects/uploaded/p${packet.id}.sturbo`;
        ...
        fs.readFile(projectFilePath, (err, data) => {
            ...
            fs.writeFile(`./projects/backup/proj${packet.id}.sturbo`, data, (err) => {
...
app.get('/api/projects/downloadRejected', async function (req, res) {
    ...
    if (project && project.removedsoft) {
        ...
        fs.readFile(`./projects/uploaded/p${project.id}.sturbo`, (err, data) => {
            if (err) {
                res.status(404);
                res.header("Content-Type", 'application/json');
    ...
    const projectDataPath = `./projects/backup/proj${packet.id}.sturbo`;
...
app.post('/api/projects/restoreRejected', async function (req, res) {
    ...
    const backupProjectFilePath = `./projects/backup/proj${projectId}.sturbo`;
    ...
    const restoredProjectFilePath = `./projects/uploaded/p${projectId}.sturbo`;
...
app.post('/api/projects/deleteRejected', async function (req, res) {
    ...
    if (project && project.removedsoft) {
        ...
        fs.unlink(`./projects/uploaded/p${projectId}.sturbo`, err => {
...
app.get('/api/projects/feature', async function (req, res) {
    ...
    const projectImage = String(`https://projects.scratchturbo.replit.app/api/pmWrapper/iconUrl?id=${project.id}&rn=${Math.round(Math.random() * 9999999)}`);
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            color: 16771677,
            url: String("https://studio.scratchturbo.replit.app/#" + String(project.id)),
            author: {
                name: String(project.owner).substring(0, 50),
                icon_url: String("https://trampoline.turbowarp.org/avatars/by-username/" + String(project.owner).substring(0, 50)),
                url: String("https://scratchturbo.replit.app/profile?user=" + String(project.owner).substring(0, 50))
...
app.get('/api/projects/delete', async function (req, res) {
    ...
    db.delete(String(packet.id));
    fs.unlink(`./projects/uploaded/p${packet.id}.sturbo`, err => {
        if (err) console.log("failed to delete project data for", packet.id, ";", err);
    })
...
app.post('/api/projects/update', async function (req, res) {
    ...
    if (Cast.isString(projectBufferData)) {
        ...
        if (buffer) {
            ...
            fs.writeFile(`./projects/uploaded/p${id}.sturbo`, buffer, (err) => {
                if (err) console.error(err);
            });
        }
        project.accepted = true;
...
app.post('/api/projects/publish', async function (req, res) {
        ...
        if (projectCodeJSON.extensions) {
            ...
            for (let extension of projectCodeJSON.extensions) {
                ...
                const trustedSources = [
                    "https://extensions-scratchturbo.replit.app/",
    ...
    fs.writeFile(`./projects/uploaded/p${id}.sturbo`, project, (err) => {
        if (err) console.error(err);
    });
...
app.get('/api/projects/getPublished', async function (req, res) {
    ...
    if (db.has(String(req.query.id))) {
        ...
        if (requestType == "file") {
            fs.readFile(`./projects/uploaded/p${req.query.id}.sturbo`, (err, data) => {
                if (err) {
                    res.status(500);
                    res.header("Content-Type", 'text/plain');
        ...
        if (requestType == "file" && requestHandleError) {
            fs.readFile(`./NoProjectFound.sturbo`, (err, data) => {
                if (err) {
                    res.status(500);
                    res.header("Content-Type", 'text/plain');
                    res.send(`<UnknownError err="${err}">`);
